...
MS_STATUS=`docker exec mysql_master sh -c 'mysql -u root -p123456 -e "SHOW MASTER STATUS"'`
...
docker exec database_slave sh -c "mysql -u root -p123456 -e 'SHOW SLAVE STATUS \G'"